	...
	case EPM_RANDOM:
		{
			...
			else if (!Sound && (!TimeMsDelayFinished || timeMs > TimeMsDelayFinished))
			{
				...
				//if (SoundFileName.size())
					Sound = SoundEngine->play3D(SoundSource, pos, false, true, true);
	...
	case EPM_LOOPING:
		{
			...
			{
				//if (SoundFileName.size())
					Sound = SoundEngine->play3D(SoundSource, pos, true, true, true);
...
void CIrrKlangSceneNode::setSoundSource(ISoundSource* soundSource)
{
...
ISoundSource* CIrrKlangSceneNode::getSoundSource() const
{
	return SoundSource;
}
