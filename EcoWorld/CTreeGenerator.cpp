...
inline f32 parseFloat( const wchar_t* &c )
{
    ...
    f32 f = 0.0f;
    swscanf_s( tmp.c_str(), L"%f", &f ); // Ancien : swscanf( tmp.c_str(), L"%f", &f );
    return f;
}
...
void CTreeGenerator::loadFromXML( io::IXMLReader* xml )
{
    ...
    RootIdRef = -1;
    SBranch* currentBranch = NULL;
    SBranchChild* currentChild = NULL;
    SLeaf* currentLeaf = NULL;
...
STreeMesh* CTreeGenerator::generateTree( s32 radialSegments, s32 seed, bool addLeaves, s32 cutoffLevel )
{
    ...
    SBranch* const root = getBranchWithId( RootIdRef );
    if ( root )
...
{
    return (s32)(getRandomFloat(seed, (float)min, (float)max) + 0.50f); // Ancien : return (s32)(getRandomFloat(seed, min, max) + 0.50f);
}
...
void CTreeGenerator::appendBranch( const core::matrix4& transform, SBranch* branch, f32 lengthScale, f32 radiusScale, s32 level, SMeshBuffer* buffer, core::list<STreeLeaf>& leaves )
{
    if ( level <= 0 )
        return; // Do nothing, this branch is invisible.
    
    // Add this branch
    const f32 length = lengthScale * getFloatFromValueRange( branch->Length, Seed++ );

	const f32 radiusEndScale = radiusScale * getFloatFromValueRange( branch->RadiusEnd, Seed++ );
    ...
    const s32 radialSegments = (level*(RadialSegments-3))/Levels + 3;
    ...
    if ( level > 1 )
    {
        const core::list<SBranchChild>::Iterator END = branch->Children.end();
        core::list<SBranchChild>::Iterator it = branch->Children.begin();
        while ( it != END )
        {
            SBranchChild* const child = &(*it);
            ++it;
            ...
            const s32 childLevel = level + getIntFromValueRange( child->RelativeLevel, Seed++ ); // RelativeLevel is usually negative, -1 in particular.
            
            const s32 numChildren = getIntFromValueRange( child->Count, Seed++ );
            
            const f32 positionRange = (child->Position.Max - child->Position.Min) / (f32)numChildren;
            
            f32 orientation = getRandomFloat( Seed++, 0, 360.0f );
            
            for ( s32 i=0; i<numChildren; ++i )
            {
                const f32 childLengthScale = lengthScale * getFloatFromValueRange( child->LengthScale, Seed++ );
                ...
                const f32 childOrientation = orientation;
                
                const f32 gravity = getFloatFromValueRange( child->GravityInfluence, Seed++ );
                
                const f32 childPitch = getFloatFromValueRange( child->Pitch, Seed++ );
                
                const f32 childPosition = getFloatFromValueRange( child->Position, Seed++ );
    ...
    if ( AddLeaves )
    {
        const core::list<SLeaf>::Iterator END = branch->Leaves.end();
        core::list<SLeaf>::Iterator lit = branch->Leaves.begin();
        while ( lit != END )
        {
            SLeaf* const leaf = &(*lit);
            ++lit;
            
            if ( !isValueInRange( level, leaf->LevelRange ) )
                continue;
            
            const s32 count = getIntFromValueRange( leaf->Count, LeafSeed++ );
            
            for ( s32 i=0; i<count; ++i )
            {
            
                const f32 width = getFloatFromValueRange( leaf->Width, LeafSeed++ );
                const f32 height = getFloatFromValueRange( leaf->Height, LeafSeed++ );
                const f32 scale = getFloatFromValueRange( leaf->Scale, LeafSeed++ );
                const f32 position = getFloatFromValueRange( leaf->Position, LeafSeed++ );
                const f32 roll = getFloatFromValueRange( leaf->Roll, LeafSeed++ );
                const f32 anchor = getFloatFromValueRange( leaf->Anchor, LeafSeed++ );
                ...
                if ( leaf->HasAxis )
                {
                    ...
                    treeLeaf.Position += leaf->Axis * height * scale * anchor * 0.5f;
                }
                else
