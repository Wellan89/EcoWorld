...
CGUIMessageBox::CGUIMessageBox(IGUIEnvironment* environment, const wchar_t* caption,
	...
	bool addTexturedModalScreen = true)
	...
	Flags(flags), MessageText(text), Pressed(false)
{
	...
	// Ajoute le modal screen obscurci à cette boîte de dialogue si nécessaire
	if (addTexturedModalScreen)
	{
		CGUITexturedModalScreen* modalScreen = new CGUITexturedModalScreen(parent, -1, environment, this);
		modalScreen->drop();
	}
}
