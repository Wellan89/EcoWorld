...
namespace SPK
{
	...
	{
		const NormalEmitter& emitter = static_cast<const NormalEmitter&>(object);	// Ancien : dynamic_cast
		Emitter::copyChildren(emitter,createBase);
		normalZone = static_cast<Zone*>(copyChild(emitter.normalZone,createBase));	// Ancien : dynamic_cast
	}
