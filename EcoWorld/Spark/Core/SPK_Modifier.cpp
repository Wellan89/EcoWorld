...
namespace SPK
{
	...
	{
		const Modifier& modifier = static_cast<const Modifier&>(object);	// Ancien : dynamic_cast
		Registerable::copyChildren(modifier,createBase);
		zone = static_cast<Zone*>(copyChild(modifier.zone,createBase));	// Ancien : dynamic_cast
	}
