...
namespace SPK
{
	...
	{
		const Group& group = static_cast<const Group&>(object);						// Ancien : dynamic_cast
		Registerable::copyChildren(group,createBase);

		model = static_cast<Model*>(copyChild(group.model,createBase));				// Ancien : dynamic_cast
		renderer = static_cast<Renderer*>(copyChild(group.renderer,createBase));	// Ancien : dynamic_cast
		...
		for (std::vector<Emitter*>::const_iterator it = group.emitters.begin(); it != group.emitters.end(); ++it)
			emitters.push_back(static_cast<Emitter*>(copyChild(*it,createBase)));	// Ancien : dynamic_cast
		for (std::vector<Modifier*>::const_iterator it = group.modifiers.begin(); it != group.modifiers.end(); ++it)
			modifiers.push_back(static_cast<Modifier*>(copyChild(*it,createBase)));	// Ancien : dynamic_cast
	}
