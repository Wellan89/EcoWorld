...
// 
//---------------------------------------------------------
CHeightMap_tmyke::CHeightMap_tmyke()
{
	m_pHeightMap = NULL;
	m_iWidth     = 0;
	m_iHeight    = 0;
...
CHeightMap_tmyke::CHeightMap_tmyke( int Width, int Height )
{
	m_iWidth  = Width;
	m_iHeight = Height;
...
// 
//---------------------------------------------------------
void CHeightMap_tmyke::Create( int Width, int Height )
{
	m_iWidth  = Width;
	m_iHeight = Height;
	m_pHeightMap = new float[ Width * Height ];
...
// 
//---------------------------------------------------------
void CHeightMap_tmyke::Scale( float fScale )
{
	float *hbuf;
	int x,y;
...
//---------------------------------------------------------------------------------------
float* CHeightMap_tmyke::ScaleHeightmap_( float *Src, int SrcWidth, int SrcHeight, 
					               int DstWidth, int DstHeight )
{
	float *Dst = new float[ DstWidth * DstHeight ];

	const float x_step = (float)SrcWidth  / (float)DstWidth;
	const float y_step = (float)SrcHeight / (float)DstHeight;

	float fXT = 0.0f, fYT = 0.0f, fXPos = 0.0f, fYPos = 0.0f;
	int iX = 0, iY = 0;
...
//---------------------------------------------------------------------------------------
bool CHeightMap_tmyke::LoadHeightMap(scene::ISceneManager* mgr, io::path fname, float HScale )
{
	float* buffer = LoadTexture32(fname, mgr);
	int width = widthT; 
	int height = heightT;
	m_iWidth = width;
	m_iHeight = height;
...
bool CHeightMap_tmyke::LoadHeightMap(scene::ISceneManager* mgr, video::IImage* heightMap, float HScale )
{
	float *fHeightMap = LoadTexture32(heightMap, mgr);
	m_iWidth	= widthT;
	m_iHeight	= heightT;

    for(int x = 0; x < widthT; ++x)
	    for(int y = 0; y < heightT; ++y)
          fHeightMap[  y * widthT + x ] *= HScale; 
...
//---------------------------------------------------------------------------------------
void CHeightMap_tmyke::GetMinMax()
{
...
//-----------------------------------------------------------------------------------------------------------
void CHeightMap_tmyke::SmoothMap()
{
	...
	TMapHauteurLisser(0, 0) = (TMapHauteur(0, 0) + TMapHauteur(0, 1) + TMapHauteur(1, 0) + TMapHauteur(1, 1)) * 0.25f;	// 1.0f / 4.0f = 0.25f
	TMapHauteurLisser((m_NbPointX-1), 0) = (TMapHauteur((m_NbPointX-1), 0) + TMapHauteur((m_NbPointX-1), 1) + TMapHauteur((m_NbPointX-2), 0) + TMapHauteur((m_NbPointX-2), 1)) * 0.25f;	// 1.0f / 4.0f = 0.25f
	TMapHauteurLisser(0, (m_NbPointY-1)) = (TMapHauteur(0, (m_NbPointY-1)) + TMapHauteur(1, (m_NbPointY-1)) + TMapHauteur(0, (m_NbPointY-2)) + TMapHauteur(1, (m_NbPointY-2))) * 0.25f;	// 1.0f / 4.0f = 0.25f
	TMapHauteurLisser((m_NbPointX-1), (m_NbPointY-1)) = (TMapHauteur((m_NbPointX-1), (m_NbPointY-1)) + TMapHauteur((m_NbPointX-2), (m_NbPointY-1)) + TMapHauteur((m_NbPointX-1), (m_NbPointY-2)) + TMapHauteur((m_NbPointX-2), (m_NbPointY-2))) * 0.25f;	// 1.0f / 4.0f = 0.25f
...
//---------------------------------------------------------------------------------------
/*void CHeightMap_tmyke::SetBorders( float fHeight )
{
	float *hbuf;
	int x,y;
...
//---------------------------------------------------------------------------------------
float CHeightMap_tmyke::GetHeightValue( int iX, int iY )
{
	if( iX >= m_iWidth ) 
		iX = m_iWidth-1;
	if( iY >= m_iHeight )
...
// récupère une hauteur terrain en fonction de coordonnées X et Y
//---------------------------------------------------------------------------------------
float CHeightMap_tmyke::GetHeightValue( float fX, float fY )
{
	int iX = (int)fX;
	int iY = (int)fY;
	/*if( iX >= m_iWidth ) 
...
//---------------------------------------------------------------------------------------
float CHeightMap_tmyke::GetInterpolateHeight( float fX, float fY )
{
    int iX, iY;
	float dX, dY;
...
// 
//---------------------------------------------------------------------------------------
/*core::vector3df CHeightMap_tmyke::GetVertexNormal( float fX, float fY, float fSizeScale )
{
...
//---------------------------------------------------------------------------------------
float* CHeightMap_tmyke::LoadTexture32(io::path filename, scene::ISceneManager* mgr)
{	
	video::IImage *heightmap = mgr->getVideoDriver()->createImageFromFile(filename);
	float *img = LoadTexture32(heightmap, mgr);
	heightmap->drop();
	return img;
}
float* CHeightMap_tmyke::LoadTexture32(video::IImage* heightmap, scene::ISceneManager* mgr)
{	
	...
	{ // pour chaque ligne de la texture
		...
		{  // pour chaque pixel de la ligne
			...
			*/
			...
			// Ajouté : (différent de ci-dessus : getPixel(v,u) != getPixel(u, v) !)
			img[v+u*heightT] = heightmap->getPixel(u, v).getLuminance();
		}
	}
...
/*
	...
	{
		char nn[128];
		sprintf_s( nn, 128, "Load texture failed: %s  \r\n", filename);
		SENDLOG(log_engine, " <<ERROR>> CHeightMap_tmyke(LoadTexture32)  \r\n", true);
...
*/
	...
	float* img2 = new float[widthT * heightT];
	for(int y=0; y<heightT; ++y)
		for(int x=0; x<widthT; ++x)
			img2[x+y*heightT] = img[(widthT * x + widthT)-1-y];
