...
#include "Game.h"	// Nécessaire pour avoir accès au timer du jeu
...
: IGUIElement(EGUIET_MODAL_SCREEN, environment, parent, id, core::recti(0, 0, parent->getAbsolutePosition().getWidth(), parent->getAbsolutePosition().getHeight())), MouseDownTime(0)
{
	#ifdef _DEBUG
...
bool CGUIModalScreen::isVisible() const
{
	...
	const core::list<IGUIElement*>::ConstIterator END = Children.end();
    for (; it != END; ++it)
    {
        if ( (*it)->isVisible() )
        {
	...
	case EET_GUI_EVENT:
		...
		case EGET_ELEMENT_FOCUSED:
			if ( event.GUIEvent.Caller == this && isMyChild(event.GUIEvent.Element) )
			{
				...
				MouseDownTime = game->deviceTimer->getRealTime();	// timer->getTime()
				return true;
			}			
...
void CGUIModalScreen::draw()
{
	...
	const u32 now = game->deviceTimer->getRealTime();
	if (now - MouseDownTime < 300 && (now / 70)%2)
	{
		core::list<IGUIElement*>::Iterator it = Children.begin();
		const core::list<IGUIElement*>::Iterator END = Children.end();
		core::recti r;
		const video::SColor c = Environment->getSkin()->getColor(gui::EGDC_3D_HIGH_LIGHT);
...
//void CGUIModalScreen::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options) const
//{
//	IGUIElement::serializeAttributes(out,options);
//}
//
////! Reads attributes of the element
//void CGUIModalScreen::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options)
//{
