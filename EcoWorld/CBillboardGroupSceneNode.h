...
// - Optimisation du rendu par la modification de la fonction CBillboardGroupSceneNode::OnRegisterSceneNode()
...
public:
	...
	virtual ~CBillboardGroupSceneNode();
    
    //! Adds a billboard.
    void addBillboard( const core::vector3df& position, const core::dimension2df& size, f32 roll=0.0f, const video::SColor& vertexColor=video::SColor(255,255,255,255) );
    
    //! Adds a billboard resitricted to an axis.
    void addBillboardWithAxis( const core::vector3df& position, const core::dimension2df& size, const core::vector3df& axis, f32 roll=0.0f, const video::SColor& vertexColor=video::SColor(255,255,255,255) );
...
private:
	...
	core::vector3df lastAbsPos;
	core::vector3df lastAbsScale;
...
public:	// Fonctions adaptées inline et rendues virtuelles
	virtual const core::aabbox3df& getBoundingBox() const	{ return BoundingBox; }

	virtual video::SMaterial& getMaterial(u32 i)			{ return MeshBuffer->Material; }

	virtual u32 getMaterialCount() const					{ return 1; }

	s32 getBillboardCount() const							{ return Billboards.size(); }

	// Ajouté : Permet une optimisation mémoire en indiquant directement la future taille du tableau des billboards (c'est-à-dire le nombre d'appels aux fonctions addBillboard et addBillboardWithAxis)
	void allocateBillboardsArray(u32 billboardsCount)
	{
		Billboards.reallocate(billboardsCount);					// Alloue suffisamment de place au tableau des billboards
		MeshBuffer->Vertices.reallocate(billboardsCount * 4);	// Alloue suffisamment de place au tableau des vertices de ce MeshBuffer
		MeshBuffer->Indices.reallocate(billboardsCount * 6);	// Alloue suffisamment de place au tableau des indices de ce MeshBuffer
	}
};
